...
async function displayTodos() {
  ...
  if (todos.length === 0) {
    ...
    todos.forEach((todo, index) =>
      console.log(chalk.cyan(`${index + 1}. ${todo.name}`))
    );
...
async function addTodo() {
  const response = await inquirer.prompt<{ todoItem: string }>({
    type: "input",
    name: "todoItem",
...
async function updateTodo() {
  ...
  let indexResponse = await inquirer.prompt<{ index: string }>({
    type: "input",
    name: "index",
  ...
  const { newTodo } = await inquirer.prompt<{ newTodo: string }>({
    type: "input",
    name: "newTodo",
